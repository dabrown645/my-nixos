...
#   -O ashift=12
#
# This more or less required fot certain things to not break:
#   -O acltype=posixacl
#
...
#   -O encryption=aes-256-cgm -O keyformat=passphrase
#
# Then we create the pool whic is mirrored over both drives and encrypted
# with ZFS native encryption (if you added those flags):
zpool create -f \
  -O mountpoint=none \
  -O atime=off \
  -O ashift=12 \
...
mkdir /mnt/{nix,home,boot,persist}
